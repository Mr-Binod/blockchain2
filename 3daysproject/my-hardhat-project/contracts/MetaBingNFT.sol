...
contract MetaBingNFT {
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    Strings.toString(bytes(metadata).length),
                    metadata
                )
            );
    }
    ...
    function getContractInfo()
        external
        view
        returns (address nftContract, address contractOwner)
    {
    ...
    function BuyNFT(
        ...
        // bingNFT.BuyNFT{value: price}(nftid, sender);
        bingNFT.BuyNFT(nftid, sender, token, price);
    }
    ...
    function cancelSale(
        ...
        bingNFT.cancelSale(nftid, token, price, sender);
    }
    ...
    function getall(
        uint item
    ) external view returns (BingNFT.Sellstake[] memory) {
